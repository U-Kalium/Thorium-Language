use thiserror::Error;
use thorium_macros::{Tokenize, symbol};

use crate::compiler::CompilerError;
#[derive(Debug, PartialEq, Clone, Tokenize)]
pub enum TokenType {
    Ident(String),
    I64,
    I32,
...
impl TokenIter {
    ...
    pub fn peek(&mut self) -> Result<Token, CompilerError> {
        if self.just_initialized {
            // self.just_initialized = false;
            if let Some(token) = self.tokens.get(self.index as usize) {
                if let Token {
            ...
            if let Some(token) = self.tokens.get(self.index as usize + 1) {
                if let Token {
                    token_type: TokenType::Eof,
                    file_offset,
                } = token

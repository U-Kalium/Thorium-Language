...
use crate::compiler::tokenizer::{Token, TokenType::{self, *}, tokenize};
...
fn parse_void_expr(tokens: &mut TokenIter, parser_ctx: &mut ParserCtx) -> Result<Expr, CompilerError> {
    ...
    match &token.token_type {
        ...
        t => Err(ParserError::ExpectedType { expected_type: Type::Void, token }.into())
    }
}
fn parse_empty_expr(tokens: &mut TokenIter, parser_ctx: &mut ParserCtx) -> Result<Expr, CompilerError> {
...
fn parse_block(tokens: &mut TokenIter, parser_ctx: &mut ParserCtx) -> Result<Expr, CompilerError> {
    ...
    if let OpenCurlyBracket = token.token_type {
        ...
        return Err(ParserError::ExpectedToken { expected_token_type: TokenType::OpenCurlyBracket, token }.into())
...
fn parse_declaration(tokens: &mut TokenIter, parser_ctx: &mut ParserCtx) -> Result<Expr, CompilerError> {
    let is_mutable;
    ...
    match token.token_type {
        ...
        _ => return Err(ParserError::ExpectedToken { expected_token_type: TokenType::Ident("".to_string()), token }.into())
